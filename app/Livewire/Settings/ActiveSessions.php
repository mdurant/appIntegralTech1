...
class ActiveSessions extends Component
{
    ...
    {
        // Asegurar que la sesión actual esté registrada
        $this->ensureCurrentSession();
    }

    public function terminateSession(string $sessionId): void
    {
        $session = UserActiveSession::where('session_id', $sessionId)
        ...
        // Si es la sesión actual de Laravel, cerrar sesión
    ...
    {
        $sessions = UserActiveSession::where('user_id', Auth::id())
            ->where('is_terminated', false)
        ...
        $otherSessions = $sessions->where('is_current', false);
    ...
    protected function ensureCurrentSession(): void
    {
        ...
        // Verificar si ya existe una sesión actual registrada
        $existingSession = UserActiveSession::where('session_id', $currentSessionId)
    ...
    {
        // Por ahora retornar una ubicación genérica
        // En producción, podrías usar un servicio como MaxMind GeoIP2 o ipapi.co
